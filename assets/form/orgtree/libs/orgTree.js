...
;(function($,window,document,undefined){
    ...
    oTree.prototype = {
        dialog: function(){
            ...
            _this = this;
            contentsHTML = '<div class="multiPickerDlg clearfix">' +
                '<div class="multiPickerDlg_left">' +
                '<div class="multiPickerDlg_left_cnt"><!--loading-->' +
                '<div class="ww_loading loading_left_wrap js_search_loading">' +
                '<span class="ww_loadingIconSmall"></span><span class="ww_loading_text">正在搜索，请稍候…</span>' +
                '</div><!--jstree--><div id="partyTree" class="multiPickerDlg_left_cnt_list">' +
                '<div class="ww_loading js_search_loading"><span class="ww_loadingIconSmall"></span><span class="ww_loading_text">正在搜索，请稍候…</span></div>' +
                '<div class="ztree_wrapper"><ul id="ztreeArea" class="ztree"></ul></div></div>' +
                '<div class="multiPickerDlg_left_mask js_left_mask"></div></div></div>' +
                '<div class="multiPickerDlg_right"><div class="multiPickerDlg_right_title">已选择的部门</div>' +
                '<div class="multiPickerDlg_right_cnt"><div class="js_right_col"><ul id="saveNode"></ul></div>' +
                '<div class="multiPickerDlg_right_no_result"><i></i>未选择部门</div></div></div></div>';

            //获取全部组织架构包括人物
            $.getJSON(this.settings.url).done(function(data){
                console.log(_this.settings.url)
                allJson = data;
        ...
        getValue: function(){
            ...
            var savedVal = this.$element.find(".field-dept").val().split(';');
            console.log('getValue',savedVal);
            var filterJson = [];
        ...
        savedJson: function(){
            // var itemList = $('.js_party_select_result_list',this).children('div');
            var itemList = this.$element.find(".js_party_select_result_list").children('div');
            ...
            //渲染外部input的值
            this.$element.find(".field-dept").val(outInputVal);
            console.log('savedJson',outInputVal);

            return jsonList;
        },
        renderOut: function(arr) {
            // $('.js_party_select_result_list',this).html('');
            this.$element.find(".js_party_select_result_list").html('');
            var wrap ='';
            var perVal = '';
            if(arr.length>0) {
            ...
            this.$element.find(".js_party_select_result_list").append(wrap);
            //渲染外部input的值
            this.$element.find(".field-dept").val(perVal);
            console.log('renderOut',perVal);
        },
