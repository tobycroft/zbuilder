...
// +----------------------------------------------------------------------
namespace tobycroft\zbuilder\form;
...
class Builder extends ZBuilder
{
    ...
    public function __construct()
    {
        ...
        $this->_vars['_token_value'] = $this->_vars['_token_name'];
    }
    ...
    public function setToken($name = '__token__', $type = 'md5')
    {
        ...
        $this->_vars['_token_value'] = $type($this->_vars['_token_name']);
        return $this;
    }
    ...
    {
        if (strpos($name, ',')) {
            [$name_from, $name_to] = explode(',', $name);
            $id_from = $name_from;
            $id_to   = $name_to;
            $id      = $name_from;
        } else {
        ...
        if (strpos($default, ',') !== false) {
            [$value_from, $value_to] = explode(',', $default);
        } else {
            $value_from = $default;
            $value_to   = '';
        }
    ...
    {
        $size = ($size != '' ? $size : config('zbuilder.upload_file_size')) * 1024;
        $ext  = $ext != '' ? $ext : config('zbuilder.upload_file_ext');
        $item = [
            'type'        => 'file',
            'name'        => $name,
            'title'       => $title,
    ...
    {
        $size = ($size != '' ? $size : config('zbuilder.upload_image_size')) * 1024;
        $ext  = $ext != '' ? $ext : config('zbuilder.upload_image_ext');
        $item = [
            'type'        => 'image',
            'name'        => $name,
            'title'       => $title,
    ...
    {
        $table_token = substr(sha1($table . '-' . $option . '-' . $key . '-' . session('user_auth.last_login_ip') . '-' . session('user_auth.last_login_time')), 0, 8);
    ...
    public function AddBuilder(string $name, ZBuilder $className)
    {
        if (!isset($this->classNames[$name]) && $className  instanceof ZBuilder) {
            $this->classNames[$name] = $className;
        } else {
            throw new Exception('类：' . $name . '已经存在或对象不是ZBuilder实例！', 7001);
        }
    }
    ...
    public function __call($methodName, $argument)
    {
        ...
        } elseif ($this->classNames[$type]) {

            if (method_exists($this->classNames[$type], 'item')) {
                $class             = new $this->classNames[$type];
                $form_item         = call_user_func_array([$class, 'item'], $argument);
                $form_item['type'] = $type;
                if (!empty($class->js)) {
                    $this->_vars['extend_js_list'][$type] = $this->parseUrl($class->js, $type);
    ...
    private function parseUrl($urls = [], $type = '')
    {
        foreach ($urls as $key => $item) {
            ...
            $urls[$key] = str_replace(array_keys(config('zbuilder.tpl_replace_string')), array_values(config('zbuilder.tpl_replace_string')), $urls[$key]);
        }
        return $urls;
    }
                            ...
                            switch ($value['type']) {
                                case 'daterange':
                                    if ($value['name_from'] == $value['name_to']) {
                                        [$group[$key]['value_from'], $group[$key]['value_to']] = $this->_vars['form_data'][$value['id']];
                                    } else {
                                ...
                                case 'time':
                                    ...
                                    if (is_numeric($group[$key]['value'])) {
                                        if ($value['type'] == 'datetime' || $value['type'] == 'time') {
                                            $group[$key]['value'] = self::format_moment($group[$key]['value'], $value['format']);
                                        } else {
                                            $group[$key]['value'] = self::format_date($group[$key]['value'], $value['format']);
                                        }
                                    }
                                    break;
                                case 'bmap':
                    ...
                    switch ($item['type']) {
                        case 'daterange':
                            if ($item['name_from'] == $item['name_to']) {
                                [$item['value_from'], $item['value_to']] = $this->_vars['form_data'][$item['id']];
                            } else {
                        ...
                        case 'time':
                            ...
                            if (is_numeric($item['value'])) {
                                if ($item['type'] == 'datetime' || $item['type'] == 'time') {
                                    $item['value'] = self::format_moment($item['value'], $item['format']);
                                } else {
                                    $item['value'] = self::format_date($item['value'], $item['format']);
                                }
                            }
                            break;
                        case 'bmap':
