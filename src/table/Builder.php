...
// +----------------------------------------------------------------------
namespace tobycroft\zbuilder\table;

use think\facade\Env;
use think\facade\View;
use think\facade\Cache;
use tobycroft\zbuilder\ZBuilder;
use tobycroft\zbuilder\table\excel\Download;
use tobycroft\zbuilder\table\excel\ExcelWriter;
...
class Builder extends ZBuilder
{
     ...
     */
    private array $_replace_right_buttons = [];
    /**
     * @var bool 有分页数据
     */
    private bool $_has_pages = true;
    /**
     * @var array 存储字段筛选选项
     */
    private array $_filter_options = [];
    /***
     * @var array 存储字段筛选列表
     */
    private array $_filter_list = [];
    /**
     * @var array 存储字段筛选类型
     */
    private array $_filter_type = [];
    /**
     * @var array 列名
     */
    private array $_field_name = [];
    /**
     * @var array 存储搜索框数据
     */
     ...
     */
    private array $_select_list_default = [];
    /**
     * @var array 行class
     */
    private array $_tr_class = [];
    /**
    ...
    public function __construct()
    {
        ...
        self::load_assets();
        $this->_controller = parse_name(request()->controller());
        $this->_action     = request()->action();
        $this->_table_name = strtolower($this->_controller);
        // 默认加载快速编辑所需js和css
     ...
     * @author 蔡伟明 <314013107@qq.com>
     */
    public function assign($name, $value = ''): object
    {
        if (is_array($name)) {
        ...
        if (!empty($columns)) {
            ...
            if (!empty($default) && is_array($default)) {
                foreach ($default as $display => $content) {
                    if (strpos($display, '|')) {
                        [$display, $filter] = explode('|', $display);
                    } else {
                        $filter = $display;
                    }
                    if (strpos($display, '.')) {
    ...
    public function addFilterList($field = '', $list = [], $default = '', $type = 'checkbox')
    {
        if ($field != '' && !empty($list)) {
            ...
            $this->_filter_list[$field]      = md5('_filter_list_' . $this->_controller . '_' . $this->_action . '_' . session('user_auth.uid') . '_' . $field);
    ...
    private function buildFilterMap($map = '')
    {
        ...
        if ($_pos !== false) {
            $_filter_content = request()->param('_filter_content');
    ...
    public function addTimeFilter($field = '', $date = '', $tips = '')
    {
        if ($field != '') {
            ...
            if (!empty($date)) {
                if (!is_array($date)) {
                    if (strpos($date, ',')) {
                        [$date_start, $date_end] = explode(',', $date);
                    } else {
            ...
            if (!empty($tips)) {
                if (!is_array($tips)) {
                    if (strpos($tips, ',')) {
                        [$tips_start, $tips_end] = explode(',', $tips);
                    } else {
    ...
    public function autoAdd($items = [], $table = '', $validate = '', $auto_time = '', $format = '', $pop = false)
    {
        if (!empty($items)) {
            ...
            // 缓存名称
            $cache_name = strtolower($this->_controller . '/add');
            // 自动插入时间
            if ($auto_time != '') {
            ...
            $form = [
                'items'     => $items,
                'table'     => $table == '' ? strtolower('_' . $this->_controller) : $table,
                'validate'  => $validate === true ? ucfirst($this->_controller) : $validate,
                'auto_time' => $auto_time,
                'format'    => $format,
                'go_back'   => request()->server('REQUEST_URI'),
            ];
    ...
    public function autoEdit($items = [], $table = '', $validate = '', $auto_time = '', $format = '', $pop = false, $extra = [])
    {
        if (!empty($items)) {
            ...
            // 缓存名称
            $cache_name = strtolower($this->_controller . '/edit');
            // 自动插入时间
            if ($auto_time != '') {
            ...
            $form = [
                'items'     => $items,
                'table'     => $table == '' ? strtolower($this->_controller) : $table,
                'validate'  => $validate === true ? ucfirst($this->_controller) : $validate,
                'auto_time' => $auto_time,
                'format'    => $format,
                'go_back'   => request()->server('REQUEST_URI'),
    ...
    private function createTableToken($table = '', $prefix = 1)
    {
        ...
        $table_token = substr(sha1($this->_controller . '-' . $this->_action . '-' . $table), 0, 8);
    ...
    public function setSearchArea($items = [], $url = '')
    {
        if (!empty($items)) {
            ...
            $this->_vars['_search_area_url'] = $url == '' ? request()->baseUrl(true) : $url;
        }
        return $this;
    }
    ...
    public function setTableName($table = '', $prefix = 1)
    {
        if ($table === true) {
            ...
            if ($this->_prefix == 2) {
                $this->_table_name = strpos($table, '/') ? $table : strtolower($table);
            } else {
    ...
    public function setRowList($row_list = null)
    {
        ...
        if (empty($this->_vars['row_list'])) {
            ...
            if (isset($params['page'])) {
                unset($params['page']);
                $url = url($this->_controller . '/' . $this->_action) . '?' . http_build_query($params);
                redirect($url);
            }
        }
        return $this;
    ...
    private function toArray($row_list)
    {
        ...
        } else if ($row_list instanceof \think\model\Collection) {
    ...
    public function setExtraHtmlFile($template = '', $tag = '', $vars = [])
    {
        ...
        $file     = Env::get('app_path') . '/view/admin/' . $this->_controller . '/' . $template . '.html';
        if (file_exists($file)) {
                    ...
                    if (!empty($this->_replace_right_buttons)) {
                        foreach ($this->_replace_right_buttons as $replace_right_button) {
                            ...
                            foreach ($replace_right_button['maps'] as $condition) {
                                ...
                                } else if (is_callable($condition[0])) {
                    ...
                    switch ($column['type']) {
                        ...
                        case 'status': // 状态
                            ...
                            if (isset($list_status[$status])) {
                                ...
                                if (strpos($list_status[$status], ':')) {
                                    [$label, $class] = explode(':', $list_status[$status]);
                                } else {
                        ...
                        case 'byte': // 字节
                            if ($row[$column['name']] === '') {
                                ...
                                $row[$column['name'] . '__' . $column['type']] = self::format_bytes($row[$column['name']], $column['param']);
                            }
                            break;
                        case 'date': // 日期
                        case 'datetime': // 日期时间
                        case 'time': // 时间
                            ...
                            if ($row[$column['name']] == '') {
                                ...
                                $row[$column['name'] . '__' . $column['type']] = self::format_time($row[$column['name']], $format);
                            }
                            break;
                        ...
                        case 'time.edit': // 可编辑时间，默认发送的是格式化好的
                            ...
                            if ($row[$column['name']] == '') {
                                ...
                                $row[$column['name'] . '__' . $column['type']] .= self::format_moment($timestamp, $format) . '</a>';
                            }
                            // 加载moment.js
                            $this->_vars['_js_files'][] = 'moment_js';
                            break;
                        ...
                        case 'img_url': // 外链图片
                            if ($row[$column['name']] != '') {
                                $row[$column['name'] . '__' . $column['type']] = '<div class="js-gallery"><img class="image" data-original="' . $row[$column['name']] . '" src="' . $row[$column['name']] . '" alt=""></div>';
                            }
                            break;
                        case 'picture': // 单张图片
                            $row[$column['name'] . '__' . $column['type']] = '<div class="js-gallery"><img class="image" data-original="' . self::get_file_path($row[$column['name']]) . '" src="' . self::get_thumb($row[$column['name']]) . '" alt=""></div>';
                            break;
                        case 'pictures': // 多张图片
                            if ($row[$column['name']] === '') {
                                ...
                                foreach ($list_img as $k => $img) {
                                    ...
                                    $imgs .= ' <img class="image" data-original="' . self::get_file_path($img) . '" src="' . self::get_thumb($img) . '">';
                                }
                        ...
                        case 'files':
                            if ($row[$column['name']] === '') {
                                ...
                                foreach ($list_file as $k => $file) {
                                    ...
                                    $files .= ' [<a href="' . self::get_file_path($file) . '">' . self::get_file_name($file) . '</a>]';
                                }
    ...
    private function compileTable()
    {
        ...
        if ($this->_vars['_select_list']) {
            foreach ($this->_vars['_select_list'] as $name => &$select) {
                // 当前url参数
                $url_params = request()->param();
                // 要搜索的字段
                $select_field = request()->param('_select_field', '');
                $select_field = $select_field != '' ? explode('|', $select_field) : [];
                // 对应的值
                $select_value = request()->param('_select_value', '');
        ...
        if ($this->_search) {
            ...
            $this->_vars['search'] = [
                ...
                'url'         => $this->_search['url'] == '' ? request()->baseUrl(true) : $this->_search['url'],
            ];
        }
        // 编译表格数据row_list的值
        $this->compileRows();
    ...
    public function fetch($template = '', $vars = [], $config = [])
    {
        ...
        \think\facade\Config::set(['asset_version' => '20180327'], 'app');
        // 后台公共模板
        View::assign('_admin_base_layout', dirname(dirname(__FILE__)) .  DIRECTORY_SEPARATOR . "baselayout.html");
        // 当前配色方案
        View::assign('system_color', config('zbuilder.system_color'));
        // 输出弹出层参数
        View::assign('_pop', request()->param('_pop'));
        $this->_template =  dirname(__FILE__) . DIRECTORY_SEPARATOR . "layout.html";
        $this->dolphin($this->_vars);
        return View::fetch($this->_template, $this->_vars, $config);
    }

    /**
     * 导出   默认是按分页实际输出数据为准，不分页需要单独自行处理
     * @param string $filename 表格文件名称
     * @param string $header 表格标题
     * @param array $vars 表格数据
     * @param array $config 工作薄名称
     * @return mixed
     */
    public function export($filename = '', $header = [], $vars = [], $Sheet1 = 'Sheet1')
    {
        // 编译表格数据
        $this->compileTable();
        ...
        $columns_header = [];
        foreach ($this->_vars['columns']  as $key => $value) {
            $columns_header[$value['name']] = $value['title'];
        }

        if (!empty($header)) {
            $header = array_merge($columns_header, $header);
        } else {
            $header = $columns_header;
        }

        if (empty($header)) {
            foreach ($this->_vars['row_list'][0]  as $key => $value) {
                $header[$key] = $key;
            }
        }
        ...
        $wExcel->writeSheet($this->_vars['row_list'], $Sheet1, $header);
        $zipname =  runtime_path() .  $filename ?: 'example.xlsx';
        $wExcel->writeToFile($zipname);
